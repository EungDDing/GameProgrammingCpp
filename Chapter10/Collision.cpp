...
float LineSegment::MinDistSq(const LineSegment& s1, const LineSegment& s2) {
	...
	else {					  // get the closest point points on the infinite lines
		sN = (b * e - c * d); 
		tN = (a * e - b * d);
...
bool Capsule::Contains(const Vector3& point) const {
	// get minimal DistSq between point and line segment
...
bool Intersect(const LineSegment& l, const Plane& p, float& outT) {
	...
	if (Math::NearZero(denom)) {
		// this only way they intersect is if start
